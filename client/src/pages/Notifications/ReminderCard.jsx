...
function ReminderCard({ notification, onMarkRead }) {
  ...
  return (
    <div className={`flex items-start gap-3 p-4 rounded-2xl bg-white dark:bg-zinc-900 border-l-4 border-indigo-500 shadow-sm transition-all hover:scale-[1.02] ${read ? 'opacity-60' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex gap-2">
          <button onClick={handleAction} className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-medium transition-colors flex items-center gap-1.5">
            <CheckCircle size={12} /> Take action
          </button>
          <button onClick={handleSnooze} className="px-3 py-1.5 bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-slate-300 rounded-lg text-xs font-medium hover:bg-slate-200 dark:hover:bg-zinc-700 transition-colors flex items-center gap-1.5">
            <Clock size={12} /> Snooze
          </button>
          {onMarkRead && (
            <button onClick={() => onMarkRead(id)} className="px-3 py-1.5 bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-700 text-slate-500 dark:text-slate-400 rounded-lg text-xs font-medium hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors flex items-center gap-1.5">
