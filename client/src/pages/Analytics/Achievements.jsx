...
const milestones = [
  { id: '10_tasks', icon: CheckCircle, label: '10 Tasks', check: (d) => d.tasks.length >= 10, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-100 dark:bg-emerald-900/30' },
  { id: '50_tasks', icon: CheckCircle, label: '50 Tasks', check: (d) => d.tasks.length >= 50, color: 'text-indigo-600 dark:text-indigo-400', bg: 'bg-indigo-100 dark:bg-indigo-900/30' },
  { id: '100_tasks', icon: CheckCircle, label: '100 Tasks', check: (d) => d.tasks.length >= 100, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-100 dark:bg-purple-900/30' },
  { id: '7_day_streak', icon: Flame, label: '7 Day Focus Streak', check: (d) => d.habits.some(h => (h.streak || 0) >= 7), color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-100 dark:bg-orange-900/30' },
  { id: '5_goals', icon: Target, label: '5 Goals Set', check: (d) => d.goals.length >= 5, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-100 dark:bg-blue-900/30' },
  { id: 'all_done', icon: Zap, label: 'All Tasks Done', check: (d) => d.tasks.length > 0 && d.tasks.every(tt => tt.status === 'done'), color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-100 dark:bg-amber-900/30' },
]
...
function Achievements({ tasks, goals, habits }) {
  ...
  const unlockedIds = milestones.filter(m => m.check(data)).map(m => m.id)
