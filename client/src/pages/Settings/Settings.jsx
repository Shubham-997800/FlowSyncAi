      ...
      <div className="space-y-6">
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4 max-w-md">
            <div>
              ...
              <input value={name} onChange={e => setName(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Email</label>
              <input value={email} disabled className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-sm cursor-not-allowed" />
            </div>
            <button type="submit" className="bg-emerald-600 text-white px-5 py-2 rounded-xl text-sm font-semibold hover:bg-emerald-700 transition">Save Changes</button>
          </form>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center justify-between max-w-md">
            ...
            <button onClick={toggle} className={`relative w-12 h-6 rounded-full transition ${dark ? 'bg-emerald-600' : 'bg-gray-300'}`}>
          ...
          <div className="space-y-3 max-w-md">
            {[
              ...
              <div key={key} className="flex items-center justify-between py-2">
                ...
                <button onClick={() => set(!state)} className={`relative w-10 h-5 rounded-full transition flex-shrink-0 ${state ? 'bg-emerald-600' : 'bg-gray-300 dark:bg-gray-600'}`}>
          ...
          <div className="space-y-3 max-w-md">
            <div className="flex items-center justify-between py-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Google Calendar Sync</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Sync your tasks with Google Calendar</p>
                </div>
              </div>
              <button onClick={() => { setGoogleSync(!googleSync); toast.success(googleSync ? 'Google Calendar disconnected' : 'Google Calendar connected (demo)') }} className={`relative w-10 h-5 rounded-full transition flex-shrink-0 ${googleSync ? 'bg-emerald-600' : 'bg-gray-300 dark:bg-gray-600'}`}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-red-200 dark:border-red-900/50">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button onClick={() => { logout(); navigate('/') }} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
