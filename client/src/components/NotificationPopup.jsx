...
const NotificationPopup = memo(function NotificationPopup() {
  ...
  return (
    ...
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(!open)} aria-label={open ? 'Close notifications' : 'Open notifications'} title="Notifications" className="relative p-2 rounded-xl text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors">
        <Bell size={18} />
        {unread > 0 && (
      ...
      {open && (
        <>
          ...
          <div className="fixed left-3 right-3 top-14 z-40 sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:w-80 sm:max-w-none bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-xl overflow-hidden">
